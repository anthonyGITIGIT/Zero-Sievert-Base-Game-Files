function scr_metti_quest_successiva()
{
    ...
    if (global.which_quest_is_shown == quest_adesso)
    {
        if (instance_exists(speak_nearest))
        {
            ...
            var j = 0;
            var k = 0;
            var _quest_array, _quest_premio;
            
            for (var i = 0; i < _n_quest; i++)
            {
                var _quest_id = speak_nearest.quest_array[i];
                
                if (_quest_id != global.which_quest_is_shown && _quest_id != -1 && _quest_id != UnknownEnum.Value_0)
                {
            ...
            for (var i = 0; i < array_length_1d(speak_nearest.quest_array); i++)
            {
                speak_nearest.quest_array[i] = UnknownEnum.Value_0;
                speak_nearest.quest_premio[i] = -1;
            }
            
            var npc_name_ = nearest_npc_id_ + "_quest";
            db_open("general");
            db_section_delete(npc_name_);
            
            for (var i = 0; i < k; i++)
            {
                speak_nearest.quest_array[i] = _quest_array[i];
                speak_nearest.quest_premio[i] = _quest_premio[i];
                db_write(npc_name_, "quest_" + string(i), _quest_array[i]);
                db_write(npc_name_, "quest_reward" + string(i), _quest_premio[i]);
            }
            
            var _quest_n = argument_count - 3;
            
            for (var i = 0; i < _quest_n; i++)
            {
                var _quest_successiva = argument[i + 3];
                var _quest_ce_gia = false;
                ...
                if (_quest_ce_gia == false)
                {
                    ...
                    db_write(npc_name_, "quest_" + string(i + k), _quest_successiva);
                    db_write(npc_name_, "quest_reward" + string(i + k), c_reward);
                }
            }

...
if (keyboard_check_pressed(ord("Q")))
{
    ...
    with (obj_bt_line_parent)
    {
        ...
        var _num = collision_line_list(x, y, _x2, _y2, obj_node_parent, true, true, _list_node_collision, false);
    ...
    with (obj_node_parent)
    {
        ...
        trace(_tree_can_be_interrupted);
        var _t_tree_child = "tree_child[i] = [ ";
        
        for (var i = 0; i < array_length(tree_child); i++)
        {
...
if (keyboard_check(ord("I")))
{
    if (mouse_check_button_pressed(mb_left))
    {
        if (instance_position(mouse_x, mouse_y, obj_node_parent))
        {
            ...
            if (global.node_type[_node_id] == UnknownEnum.Value_4)
            {
                if (_id.tree_can_be_interrupted == true)
                    _id.tree_can_be_interrupted = false;
                else
                    _id.tree_can_be_interrupted = true;
            }
        }
    }
}
    ...
    if (mouse_check_button_pressed(mb_left))
    {
        if (instance_position(mouse_x, mouse_y, obj_node_parent))
        {
            ...
            else
            {
                ...
                trace("line created");
                trace(string(first_x) + " , " + string(first_y));
            }
        }
        else
        {
...
{
    if (instance_position(obj_cursor.aa_x, obj_cursor.aa_y, obj_bt_line))
    {
        var _id = instance_position(obj_cursor.aa_x, obj_cursor.aa_y, obj_bt_line);
...
if (instance_position(obj_cursor.aa_x, obj_cursor.aa_y, obj_node_parent))
{
    var _id = instance_position(obj_cursor.aa_x, obj_cursor.aa_y, obj_node_parent);
    over_node_id = _id;
}

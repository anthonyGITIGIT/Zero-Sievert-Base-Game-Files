if (can_draw_surface == true)
{
    if (!surface_exists(surface_shadow))
    {
                    ...
                    for (var yy = 1; yy < (o.grid_tile_h - 1); yy++)
                    {
                        if (ds_grid_get(o.grid_tile, xx, yy) != -1)
                        {
                            ...
                            if (tile_sotto && !tile_sopra && !tile_sopra_water && !tile_water_a)
                                draw_rectangle_color(xx * 16, yy * 16, (xx * 16) + 16, (yy * 16) + 32, c_black, c_black, c_black, c_black, false);
                        }
                    }
                }
            }
        ...
        {
            if (object_index != obj_solid && object_index != obj_train)
            {
                if (visible == true)
                    scr_draw_shadow_decor();
            }
            ...
            with (obj_solid)
            {
                if (object_index == obj_solid)
                {
                    if (y > (obj_map_generator.grid_tile_h * 16))
                        draw_rectangle_color(x, y, x + 16, y + 32, c_black, c_black, c_black, c_black, false);
                }
            }
        }
